...
function applyConfigToDom() {
  ...
  try {
    ...
    const desc = `HVAC service today in ${SITE_CONFIG.primaryCity}. Confirm availability in under 60 seconds.`;
    ...
    items.forEach((t, idx) => {
      ...
      slide.innerHTML = `
        <div class="testimonial__meta">
          ...
          <div class="verified">Recent homeowner feedback</div>
        </div>
...
function validate(form, { showErrors } = { showErrors: false }) {
  ...
  // Name is optional (reduces friction). We only require reliable contact + location.
  if (showErrors) setError(form, "firstName", "");
